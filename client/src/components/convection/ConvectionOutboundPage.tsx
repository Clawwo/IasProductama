...
export function ConvectionOutboundPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (draftKind && draftKind !== "CONVECTION_OUTBOUND") return;

      setReceiver(typeof payload.receiver === "string" ? payload.receiver : "");
      setDate(
        ...
        ? payload.lines
            .map((l) => ({
              ...
              stockAvailable:
                allItems.find(
                  (it) =>
                    it.code === (typeof l.code === "string" ? l.code : ""),
                )?.stockBase ?? 0,
              hasWarning: false,
            }))
            .filter((l) => l.code)
      ...
      if (incomingLines.length > 0) {
        setLines(
          incomingLines.map((line) => ({
            ...line,
            hasWarning: line.qty > line.stockAvailable,
          })),
        );
      }
  ...
  const handleAddLine = () => {
    ...
    const item = allItems.find((i) => i.code === lineItem.code);
    const stockAvailable = Number(item?.stockBase ?? 0);

    // Check if qty exceeds stock (only warn, don't block)
    const hasWarning = qty > stockAvailable;
  ...
  const handleSubmit = async () => {
    if (!receiver.trim()) {
      setFormError("Penerima harus diisi");
      return;
    }
    ...
    try {
      ...
      setSubmitMessage("Barang keluar berhasil disimpan!");
      pushToast(
        "default",
        "Data tersimpan",
  ...
  // Check if any line has warning
  const hasAnyWarning = lines.some((line) => line.hasWarning);
